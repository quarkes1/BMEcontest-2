#!/bin/bash
# 5 折全链：重建缓存（gate 全体会话打分）→ 重训排序头（meta 含 gate_prob）→ v2 双网格解码
...
for k in $(seq $START 4); do
  ...
  python -u scripts/rank_events_v2.py --fold $k --prior-grid 15m || exit 1
  python -u scripts/rank_events_v2.py --fold $k --prior-grid 60m || exit 1
done
echo "===== 汇总 ====="
