...
from src.pipeline.artifacts import PROMOTION_F1_FLOOR, load_event_stack_bundle, verify_bundle_manifest
...
def _run_smoke(script: Path, bundle: Path, payload: Path, output: Path) -> bytes:
    completed = subprocess.run(
        [sys.executable, str(script), "--bundle", str(bundle), "--input-features", str(payload), "--output", str(output), "--device", "cpu"],
        cwd=_ROOT,
        text=True,
    ...
    if "resolved_device=cpu" not in completed.stdout:
        raise RuntimeError("isolated inference smoke did not report resolved CPU device")
...
def _verify_fixture_parity(staging: Path, source_bundle: Path) -> None:
    ...
    payload = staging / ".fixture.json"
    repo_output = staging / ".repo-output.json"
    dist_output = staging / ".dist-output.json"
    ...
    try:
        ...
        for path in (payload, repo_output, dist_output):
            if path.exists():
                path.unlink()
...
def package_event_stack(*, bundle_path: Path, destination: Path) -> Path:
    """Package one verified deployment bundle through a same-parent atomic swap."""

    bundle_path = Path(bundle_path).absolute()
    destination = Path(destination).absolute()
    if destination.name != "event_stack":
        raise ValueError("destination must be the exact dist/event_stack directory")
    ...
    try:
        ...
        subprocess.run([sys.executable, str(staging / "predict_event_stack.py"), "--help"], check=True, cwd=_ROOT, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
