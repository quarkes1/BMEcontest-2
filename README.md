...
**当前最优（滑窗管线 v5.1，per-fold TCN 模式选择，全局官方口径）**：

| fold | F1 | sens | ppv | TP/eligible | TCN 模式 |
|---|---|---|---|---|---|
| 0 | 0.512 | 0.478 | 0.550 | 11/23 | TCN |
| 1 | **0.767** | 0.742 | 0.793 | 23/31 | TCN |
| 2 | 0.517 | 0.556 | 0.484 | 15/27 | TCN |
| 3 | 0.540 | 0.531 | 0.548 | 17/32 | TCN |
| 4 | 0.750 | 0.750 | 0.750 | 30/40 | **NO_TCN** |
| **均值** | **~0.617** | | | | |
| 全局聚合 | **0.632** | 0.627 | 0.636 | 96/153 | |

v5.1（较 v5 0.615）：fold2 bag TCN no_meal 一致性微升（0.508→0.517）；
fold0/2 攻坚结论：全量无餐复核负样本（765 会话）使复核保守化净降
（fold0 0.476/fold2 0.438）——150 会话为甜点。

v5 关键改进（较 v4.2 0.595）：per-fold TCN 模式选择——fold4 的 TCN 滑窗分
域偏移有害（NO_TCN 两次复现 +0.10：0.649→0.750）；fold0-3 保留 TCN（+0.01-0.05）。

v4.2 关键改进（较 v4 0.587）：①eligible 收紧（餐时段窗覆盖 ≥120s——与官方
IMU 质量合格口径对齐，数据碎片/缺口餐不罚检测器：fold2 +0.047）；②无餐会话
复核负样本 60→150（fold2 +0.032、fold3 +0.009）。
...
CV 均值与组内测试分的差异经全局 OOF 阈值对照实验（5 折候选 pooled 选单一阈值，
聚合 F1 0.595 ≈ per-fold 0.592）确认**非阈值选择协议所致**；差异源于数据固有
难度（fold0/2 窗级判别弱与复核域漂移——fold2 全数据复核器系统性低分，per-fold
复核器 0.508 为其最优配置；fold4 全局阈值达 0.737）。
