...
**当前最优（滑窗管线 v3，全局官方口径 5 折均值）**：

| fold | F1 | sens | ppv | TP/eligible | TCN 模式 |
|---|---|---|---|---|---|
| 0 | 0.426 | 0.400 | 0.455 | 10/25 | bag |
| 1 | **0.667** | 0.594 | 0.760 | 19/32 | bag |
| 2 | 0.373 | 0.412 | 0.341 | 14/34 | 单模型 |
| 3 | 0.429 | 0.364 | 0.522 | 12/33 | bag |
| 4 | 0.565 | 0.571 | 0.558 | 24/42 | 单模型 |
| **均值** | **~0.49** | | | | |

v3 改进（较 v2 0.468）：TCN 深度分按折选择 bag（5 模型 sigmoid 平均）或单模型
——弱折（0/1/3）bag 降方差显著提升（fold0 +0.058、fold1 突破 0.65、fold3 +0.029）；
强模型折（2/4）单模型更优（bag 稀释）。fold1 单折 **0.667** 为当前最好。

v2 改进（较 v1 0.41）：复核训练纳入无餐会话候选负样本（修复 fold2 复核失效）、
复核特征 +TCN 深度分（窗级相关 0.26 互补）+ 事件时刻先验、窗模型融合
TCN 分 + 时刻先验（62→64 特征）。逐折产物 outputs/slide_verifier_fold{k}.json。
